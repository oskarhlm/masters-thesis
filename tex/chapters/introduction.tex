...
The release of OpenAI's ChatGPT in November, 2023\todo{ref} generated a hype within the general population and chat-based systems are flourishing. Such \glspl{acr:llm} provide a natural interface between human and machine, one in which technical know-how is not necessarily required in order to perform technical tasks, as modern \glspl{acr:llm} are often able to interpret the user's intent regardless of the preciseness of their prompt.

\acrshort{acr:gis} analysis has traditionally been reserved for \acrshort{acr:gis} \textit{experts}.
...
\begin{enumerate}
    \item Can an \gls{acr:llm}-based system answer advanced \acrshort{acr:gis} problems? \label{rq:gis-question-answering}
    \item What are state-of-the-art methods of creating autonomous \acrshort{acr:llm}-based agents? \label{rq:llm-agents-sota}
    \item What are core challenges in developing such systems? \label{rq:development-challenges}
\end{enumerate}
...
The specialization project which this thesis is based upon \citep{holmLLMsDeathGIS2023} was of a theoretical character, though it was supplemented by some simple experiments based around direct prompting with ChatGPT. This master thesis will, however, be more practical. As explained in \autoref{sec:goals-and-research-questions}, the overarching goal is to create a \enquote{proof of concept}. The resulting prototype will then be used to answer the research questions.
