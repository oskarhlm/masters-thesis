...
\section[Walking Into Dead Ends]{Walking Into Dead Ends}
\label{sec:dead-ends}

% \autoref{fig:duration-box-plot} in \autoref{tbl:test-results-quantitative} clearly shows that the \acrshort{acr:sql} agent has lower latency (time taken to present an answer) compared to the two other agents. Time-expensive Python imports was explained as the main contributor to high latency with the Python agent, and the same issue is likely present with the \acrshort{acr:ogc} \acrshort{acr:api} Features agent, though it is not as severe for the latter. 
