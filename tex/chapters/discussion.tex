...
\begin{figure}
    ...
    \caption[Using the multi-modal GPT-4 model to identify errors in image of generated road map layer]{ChatGPT's multi-modal \acrshort{acr:gpt}-4 correctly identifying that the map layer intended to show high-speed roads in Oslo in fact \enquote{extends far beyond Oslo}}
...
\begin{figure}
    ...
    \caption[Architecture a for multi-agent implementation for GeoGPT's OGC API Features agent]{Illustration for multi-agent implementation for GeoGPT's \acrshort{acr:ogc} \acrshort{acr:api} Features agent, where an agent supervisor takes in a user message and selects which sub-agent is to solve the next sub-task}
