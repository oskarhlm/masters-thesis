...
\begin{lstlisting}[
    language=Python,
    caption=Python code aimed at computing the difference between the Oslo outline and water features within it,
...
On the other hand, the expert-level prompt provided the necessary guidance for GeoGPT for this specific task, steering it clear of the issues it encountered with the novice-level prompt. As stated by OpenAI themselves\footnote{\url{https://platform.openai.com/docs/guides/prompt-engineering/strategy-write-clear-instructions}}, \enquote{some tasks are best specified as a sequence of steps}. Furthermore, they say that writing explicit steps required to solve a tasks \enquote{makes it easier for the model to follow them}.\todo{move to discussion?}
